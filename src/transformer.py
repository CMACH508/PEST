...
if __name__ == '__main__':
    ...
    transformer = Transformer_structure(dim, depth, heads, dim_head, mlp_dim, dropout).cuda()
    x = torch.rand([1, 4, 1024]).cuda()
    m = torch.ones([1, 4], dtype=bool).cuda()
    m[:, :2] = False
    # print(m)
    d = torch.rand([1, 4, 4]).cuda()
    out = transformer(x, d, m)
    print(out.shape)
